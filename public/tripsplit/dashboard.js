(function (global) {
  ...
  function personalPayerShares(expense, logic) {
    return logic.isFundExpense(expense) ? {} : logic.getExpensePayerShares(expense);
  }
  function summarizeTrip(trip, logic) {
    ...
    expenses.forEach((item) => { const category = String(item.category || "Khác"); categories[category] = (categories[category] || 0) + toAmount(item.amount); Object.entries(personalPayerShares(item, logic)).forEach(([memberId, amount]) => { payers[memberId] = (payers[memberId] || 0) + toAmount(amount); }); });
  ...
  function aggregate(portfolio, logic, period = "year", anchor = new Date()) {
    ...
    included.forEach(({ trip, expense }) => { const category = String(expense.category || "Khác"); categories[category] = (categories[category] || 0) + toAmount(expense.amount); Object.entries(personalPayerShares(expense, logic)).forEach(([memberId, amount]) => { const member = (trip.members || []).find((item) => item.id === memberId); const key = `${trip.id}:${memberId}`; if (!payers[key]) payers[key] = { key, name: member?.name || "Không xác định", tripName: trip.name, amount: 0 }; payers[key].amount += toAmount(amount); }); });
