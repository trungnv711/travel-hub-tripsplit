(() => {
  ...
  function renderHome() {
    ...
    dom.homeContributorList.innerHTML = metrics.payers.length ? metrics.payers.slice(0, 6).map((item, index) => `<article class="contributor-item"><span class="contributor-rank">${index + 1}</span><div><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.tripName)}</small></div><div class="contributor-thanks" title="Lời cảm ơn theo tiền cá nhân đã ứng">${"🙏".repeat(item.thanks)}</div><span class="contributor-amount">${formatCurrency(item.amount)}</span></article>`).join("") : '<div class="home-empty">Chưa có khoản cá nhân tự trả trong khoảng thời gian này.</div>';
